<template>
    <div class="login">
        ...
        <div class="mui-content-padded">
            <button id='login' class="mui-btn mui-btn-block mui-btn-primary login-btn" @click.prevent="register">登录</button>
            <!-- <h6>*未注册用户直接登录即可完成登录</h6> -->
        </div>
        <router-link class="btnRegister" to="/user/register">立即注册</router-link>
    ...
    methods: {
        ...
        getVerifyCode() {
            if (this.rightPhoneNumber) {
                ...
                //获取验证码接口
                this.$http.post("api/getCode?").then(result => {
            ...
            this.$http
                ...
                .then(result => {
                    if (result.body.state === 1) {
                        ...
                        //登录成功保存用户信息
                        var userinfo = [];
                        userinfo.push(result.body.result)
                        localStorage.setItem("dt_userinfo",JSON.stringify(userinfo));
