...
if (!bdShare.ApiPVLogger) {
	(function(J) {
		...
		(function() {
			...
			B.browser.safari = /(\d+\.\d)?(?:\.\d)?\s+safari\/?(\d+\.\d+)?/i.test(N) && !/chrome/i.test(N) ? +(RegExp["\x241"] || RegExp["\x242"]) : undefined
		})();
		B.browser.ie = B.ie = /msie (\d+\.\d+)/i.test(navigator.userAgent) ? (document.documentMode || +RegExp["\x241"]) : undefined;
		B.browser.opera = /opera(\/| )(\d+(\.\d+)?)(.+?(version\/(\d+(\.\d+)?)))?/i.test(navigator.userAgent) ? +(RegExp["\x246"] || RegExp["\x242"]) : undefined;
		B.dom = B.dom || {};
		(function() {
		...
		var G = (function() {
			var N = [];
			return function(O) {
				var Q = "bdsharelog__" + (new Date()).getTime(),
					P = N[Q] = new Image();
		...
		var E = function(Q) {
			var O = /(\d+\-[a-zA-Z\d]+\-\d+\-\d+\-[a-f\d]+)/ig,
				//N = (L.href.match(O) || "").toString();
				N = ((window.lemmaUrl ? window.lemmaUrl.match(O) : L.href.match(O)) || "").toString();
			var P = "http://nsclick.baidu.com/v.gif?pid=307&type=3071&sign=" + N + "&desturl=" + encodeURIComponent(document.referrer) + "&";
			G(P + Q)
		};
		...
		var I = (function() {
			...
			return function() {
				if (bdShare.velocity && (!bdShare.velocity.mainJsLoaded || !bdShare.velocity.cssLoadEnd) && N++ < 3) {
					...
					if (bdShare.velocity) {
						var O = (Math.max(bdShare.velocity.cssLoadEnd, bdShare.velocity.mainJsLoaded) - bdShare.velocity.start) || 0;
